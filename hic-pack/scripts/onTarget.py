...
def usage():
    ...
    print(
        "[-c/--cis] <Report only capture-capture interactions. Otherwise both capture-capture and capture-reporter interactions are returned>")
...
def load_bed(in_file, verbose=False):
    ...
    for line in bed_handle:
        ...
        try:
            ...
            print("Warning : wrong input format in line", nline, ". Not a BED file !?", file=sys.stderr)
            continue
...
if __name__ == "__main__":
    ...
    with open(inFile) as vp_handle:
        ...
        if statsFile is not None:
            if verbose:
                print("## Writing stats in '", statsFile, "'...", file=sys.stderr)
            f = open(statsFile, 'a')
