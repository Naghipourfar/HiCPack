...
function help {
    usage;
    echo 
    echo "Generate JuiceBox input file from HiC-Pack results"
    echo "See http://www.aidenlab.org/juicebox/ for details about Juicebox"
    echo "---------------"
    echo "OPTIONS"
    echo
    echo "   -i|--input VALIDPAIRS : allValidPairs file generated by HiC-Pack >= 2.7.5"
    echo "   -g|--gsize GSIZE : genome size file used during HiC-Pack processing"
    echo "   -j|--jar JUICERJAR : path to juicebox_clt.jar file"
    echo "   [-r|--resfrag] RESFRAG : restriction fragment file used by HiC-Pack"
