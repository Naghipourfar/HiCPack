...
function usage {
    echo -e "usage : hicpro2GOTHiC -i VALIDPAIRS -g GSIZE -j JUICERJAR [-r RESFRAG] [-t TEMP] [-o OUT] [-h]"
...
function help {
    usage;
    echo 
    echo "Generate JuiceBox input file from HiCPack results"
    echo "See http://www.aidenlab.org/juicebox/ for details about Juicebox"
    echo "---------------"
    echo "OPTIONS"
    echo
    echo "   -i|--input VALIDPAIRS : allValidPairs file generated by HiCPack >= 2.7.5"
    echo "   -g|--gsize GSIZE : genome size file used during HiCPack processing"
    echo "   -j|--jar JUICERJAR : path to juicebox_clt.jar file"
    echo "   [-r|--resfrag] RESFRAG : restriction fragment file used by HiCPack"
...
#if [[ $nbfields == "12" ]]; then
#    echo -e "HiCPack format > 2.7.5 detected ..."
#
#elif [[ $nbfields == "8" ]]; then
#    echo -e "HiCPack format < 2.7.6 detected ..."
