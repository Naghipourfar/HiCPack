...
def read_HiCdata(filename, header=0, footer=0, clean_nans=True, smooth_noise=0.5, ins_window=5, rel_window=8,
                 plotInsulation=True, plotTadDomains=False, randomBins=False):
    '''
    ...
    if not randomBins and len(matrix[:, 1]) != len(matrix[1, :]):
        print(str(len(matrix[:, 1])), str(len(matrix[1, :])))
        print('unbalanced matrix(' + filename + ')! input should be a square matrix', file=sys.stderr)
        raise SystemExit

    if plotInsulation or plotTadDomains and not randomBins:
        nums, tricks = insulation(matrix, ins_window, rel_window)
    else:
        nums = [];
        tricks = [];
...
def read_sparseHiCdata(filename, chromosome, bedFile, startBin, endBin, wholeGenome=False, smooth_noise=0.5,
                       ins_window=5, rel_window=8, plotInsulation=True, plotTadDomains=False, randomBins=False,
                       anchor=0):
    '''
...
def read_Cooldata(filename, chromosome, resolution, startBin, endBin, wholeGenome=False, smooth_noise=0.5, ins_window=5,
                  rel_window=8, plotInsulation=True, plotTadDomains=False, randomBins=False):
    import cooler

    c = cooler.Cooler(filename)
    if resolution != c.binsize:
        print
        'Be aware: resolution is not matching with the cool file!'
        raise SystemExit

    start = startBin * resolution
    if endBin == 0:
        print
        'Be aware: you did not enter a valid genomic range!'
    ...
    if plotInsulation or plotTadDomains and not wholeGenome:
        nums, tricks = insulation(matrix, ins_window, rel_window, False, startBin, True)
    else:
        nums = [];
        tricks = [];
    ...
    for line in fone.xreadlines():
        ...
        if tags[0] == chromosome:
            ...
            if len(tags) > 3:
                ...
                if len(tags) > 4:
                    hex = '#%02x%02x%02x' % (
                        int(tags[4].split(',')[0]), int(tags[4].split(',')[1]), int(tags[4].split(',')[2]))
                    colors.append(hex)
                    if len(tags) > 5:
                        texts.append(tags[5])
...
def read_peakFile(filename, resolution, chromosome):  # add stopping after certain chromosome passed
    ...
    for line in fone.xreadlines():
        ...
        if tags[0] == chromosome and tags[3] == chromosome:
            ...
            if len(tags) > 5:
                hex = '#%02x%02x%02x' % (
                    int(tags[6].split(',')[0]), int(tags[6].split(',')[1]), int(tags[6].split(',')[2]))
                colors.append(hex)
        ...
        if tags[0] == chromosome and int(tags[1]) > start and int(tags[2]) < end:
            ...
            for x in range(1, len(cols)):
                if x % 2 == 1 and cols[x].replace('[', '').replace(']', '').replace(' ', '') not in y_dict.keys():
                    ...
                    y_dict[cols[x].replace('[', '').replace(']', '').replace(' ', '')].append(
                        float(cols[x - 1].replace('[', '').replace(']', '').replace(' ', '')))
                elif x % 2 == 1:
                    y_dict[cols[x].replace('[', '').replace(']', '').replace(' ', '')].append(
                        float(cols[x - 1].replace('[', '').replace(']', '').replace(' ', '')))
                ...
                if len(row_list) == 0:
                    ...
                    if prev_start > int(tags[1]):
                        ...
                        if len(tags) > 5:
                            ...
                            if len(tags) > 7:
                                hex = '#%02x%02x%02x' % (
                                    int(tags[7].split(',')[0]), int(tags[7].split(',')[1]), int(tags[7].split(',')[2]))
...
def insulation(matrix, w=5, tadRange=10, triple=False, mstart=0, cooler=False):
    ...
    for i in range(start, end, 1):
        ...
        for j in range(i, i + w):
            if j == end:
                ...
                if triple or cooler:
                    if isnan(matrix[j, j - 2 * counter]):
                        diag += 0  # pad with zeros for nan
                    else:
                        diag += matrix[j, j - 2 * counter]
                else:
                    if isnan(matrix[j][j - 2 * counter]):
                        diag += 0  # pad with zeros for nan
                    else:
    ...
    for item in range(0, len(regions) - 1):
        if item == 0:  # check the first boundary
            if len(np.nonzero(scores[slice(regions[item], regions[item + 1])])[0]) < regions[item + 1] - regions[
                item] - w / 2 - 1:
                ...
                if regions[item] + current[-1] not in pBorders and regions[item] + current[-1] + 1 not in regions:
                    pBorders.append(regions[item] + current[-1])
                elif regions[item] + current[-1] not in pBorders and regions[item] + current[-1] + 1 in regions:
                    pBorders.append(regions[item] + current[-2])  # to get closer bin but needs to be improved
            else:
                if regions[item] not in pBorders and regions[item] + 1 not in regions: pBorders.append(regions[item])
        else:
            current = np.nonzero(scores[slice(regions[item], regions[item + 1])])[0]
            if len(current) >= regions[item + 1] - regions[item] - w / 2 - 1:
                ...
                if regions[item] not in pBorders and regions[item] + 1 not in regions: pBorders.append(regions[item])
                if regions[item] + current[0] not in pBorders and regions[item] + current[
                    0] + 1 not in regions: pBorders.append(regions[item] + current[0])
                current = np.nonzero(scores[slice((regions[item] + tadRange), regions[item + 1])])[0]
                if len(current) == 0:
                    if regions[item + 1] not in pBorders and regions[item] + 1 not in regions: pBorders.append(
                        regions[item + 1])
                else:
                    if regions[item] + tadRange + current[0] not in pBorders and regions[item] + tadRange + current[
                        0] + 1 not in regions: pBorders.append(regions[item] + tadRange + current[0])
                    if regions[item + 1] not in pBorders and regions[item + 1] + 1 not in regions: pBorders.append(
                        regions[item + 1])
...
def HiCplotter(files=[], names=[], resolution=100000, chromosome='', output='', histograms=[], histLabels=[],
               fillHist=[], histMax=[], verbose=False, fileHeader=0, fileFooter=0, matrixMax=0, histColors=[],
               barPlots=[], barLabels=[], plotGenes='', superImpose=False, anchor=0, anchorMax=0, \
               start=0, end=0, tileLabels=[], tilePlots=[], tileColors=[], tileText=False, arcLabels=[], arcPlots=[],
               arcColors=[], peakFiles=[], epiLogos='', window=5, tadRange=8, tripleColumn=False, bedFile='',
               barColors=[], dPixels=200, compareEx='', compareSm='', upSide=True, Cooler=False, \
               smoothNoise=0.5, cleanNANs=True, plotTriangular=True, plotTadDomains=False, randomBins=False,
               wholeGenome=False, plotPublishedTadDomains=False, plotDomainsAsBars=False, imputed=False, barMax=[],
               spine=True, plotDomainTicks=True, triangularHeight=False, notLog=False, \
               highlights=0, lHighlights=0, highFile='', heatmapColor=3, highResolution=True, plotInsulation=True,
               plotCustomDomains=False, publishedTadDomainOrganism=True, customDomainsFile=[], compare=False,
               pair=False, domColors=[], oExtension='', geneLabels=True, dark=False):
    '''
    ...
    if superImpose:
        fig.subplots_adjust(hspace=0.48, wspace=1.25)
    else:
    ...
    for exp in range(0, rlength):
        ...
        if not tripleColumn and not Cooler:
            matrix, nums, tricks = read_HiCdata(files[exp], fileHeader, fileFooter, cleanNANs, smoothNoise, window,
                                                tadRange, plotInsulation, plotTadDomains, randomBins)
            ...
            if exp == 0:
                ...
                print
                len(matrix), mlength
                print >> sys.stderr, 'unbalanced matrix size of ' + files[exp] + ' compared to ' + files[
                    0] + ' ! matrix sizes should be equal'
                raise SystemExit
            fourClike = []
            ...
            matrix, nums, tricks = read_Cooldata(files[exp], chromosome, resolution, start, end, wholeGenome,
                                                 smoothNoise, window, tadRange, plotInsulation, plotTadDomains,
                                                 randomBins)
        else:
            if bedFile == '':
                print >> sys.stderr, 'an annotation bed file is required for triple-column sparse input.'
                raise SystemExit
            matrix, nums, tricks, clength, fourClike = read_sparseHiCdata(files[exp], chromosome, bedFile, start, end,
                                                                          wholeGenome, smoothNoise, window, tadRange,
                                                                          plotInsulation, plotTadDomains, randomBins,
                                                                          anchor)
        ...
        if not wholeGenome:
            if heatmapColor < 5:
                if not notLog:
                    with np.errstate(divide='ignore'):
                        img = ax1.imshow(log2(matrix), cmap=plt.get_cmap(cmaps[heatmapColor]), origin=orientation,
                                         interpolation="nearest", extent=(int(start or 1) - 0.5, \
                    ...
                    with np.errstate(divide='ignore'):
                        img = ax1.imshow(matrix, cmap=plt.get_cmap(cmaps[heatmapColor]), origin=orientation,
                                         interpolation="nearest", extent=(int(start or 1) - 0.5, \
                                                                          int(start or 1) + length - 0.5,
                                                                          int(start or 1) - 0.5,
                                                                          int(start or 1) + length - 0.5),
                ...
                if not notLog:
                    with np.errstate(divide='ignore'):
                        img = ax1.imshow(log2(matrix), cmap=cmap, origin=orientation, interpolation="nearest",
                                         extent=(int(start or 1) - 0.5, \
                                                 int(start or 1) + length - 0.5, int(start or 1) - 0.5,
                                                 int(start or 1) + length - 0.5), aspect='auto')
                else:
                    with np.errstate(divide='ignore'):
                        img = ax1.imshow(matrix, cmap=cmap, origin=orientation, interpolation="nearest",
                                         extent=(int(start or 1) - 0.5, \
                                                 int(start or 1) + length - 0.5, int(start or 1) - 0.5,
                                                 int(start or 1) + length - 0.5), aspect='auto')
            else:
                if not notLog:
                    with np.errstate(divide='ignore'):
                        img = ax1.imshow(log2(matrix), origin=orientation, interpolation="nearest",
                                         extent=(int(start or 1) - 0.5, \
                                                 int(start or 1) + length - 0.5, int(start or 1) - 0.5,
                                                 int(start or 1) + length - 0.5), aspect='auto')
                else:
                    with np.errstate(divide='ignore'):
                        img = ax1.imshow(matrix, origin=orientation, interpolation="nearest",
                                         extent=(int(start or 1) - 0.5, \
                                                 int(start or 1) + length - 0.5, int(start or 1) - 0.5,
                                                 int(start or 1) + length - 0.5), aspect='auto')
        else:
            if heatmapColor < 5:
                with np.errstate(divide='ignore'):
                    img = ax1.imshow(log2(matrix), cmap=plt.get_cmap(cmaps[heatmapColor]), interpolation="nearest",
                                     extent=(int(start or 1) - 0.5, \
                                             int(start or 1) + length - 0.5, int(start or 1) - 0.5,
                                             int(start or 1) + length - 0.5), aspect='auto')
            elif heatmapColor == 3:
                ...
                with np.errstate(divide='ignore'):
                    img = ax1.imshow(log2(matrix), interpolation="nearest", extent=(int(start or 1) - 0.5, \
                                                                                    int(start or 1) + length - 0.5,
                                                                                    int(start or 1) - 0.5,
                                                                                    int(start or 1) + length - 0.5),
                                     aspect='auto')
        ...
        if len(peakFiles) > 0:
            ...
            for citem in range(0, len(origin_x)):
                if len(colors) == 0:
                    circle = Circle((origin_x[citem], origin_y[citem]), radius[citem], facecolor='none',
                                    edgecolor='black', linewidth=1, alpha=0.85)
                else:
                    circle = Circle((origin_x[citem], origin_y[citem]), radius[citem], facecolor='none',
                                    edgecolor=colors[citem], linewidth=3, alpha=0.85)
        ...
        if len(h_start) > 0:
            if highlights:
                for item in range(0, len(h_start)):
                    if dark:
                        ax1.axvspan(h_start[item], h_end[item], facecolor='#78D400', alpha=0.30, linestyle='dashed')
                    else:
                        ax1.axvspan(h_start[item], h_end[item], facecolor='g', alpha=0.10, linestyle='dashed')
            if lHighlights:
                for item in range(0, len(h_start)):
                    if dark:
                        ax1.axhspan(h_start[item], h_end[item], facecolor='#78D400', alpha=0.30, linestyle='dashed')
                    else:
                        ax1.axhspan(h_start[item], h_end[item], facecolor='g', alpha=0.10, linestyle='dashed')
        ...
        if plotTriangular:
            ...
            matrix = [];
            if not triangularHeight:
                height = length / 5
            else:
                if triangularHeight <= length:
                    height = triangularHeight
                else:
                    height = length / 2
            ...
            if heatmapColor < 5:
                with np.errstate(divide='ignore'):
                    img = ax2.imshow(log2(dst), origin=orientation, cmap=plt.get_cmap(cmaps[heatmapColor]),
                                     interpolation="nearest", extent=(int(start or 1) - 0.5, \
                                                                      int(start or 1) + length - 0.5,
                                                                      int(start or 1) - 0.5,
                                                                      int(start or 1) + length - 0.5), aspect='auto')
                ...
                with np.errstate(divide='ignore'):
                    img = ax2.imshow(log2(dst), origin=orientation, interpolation="nearest",
                                     extent=(int(start or 1) - 0.5, \
                                             int(start or 1) + length - 0.5, int(start or 1) - 0.5,
                                             int(start or 1) + length - 0.5), aspect='auto')
            dst = [];
            ...
            rowcounter += 2
            if numOfrows <= rowcounter and not randomBins:
                ax2.set_xlabel('Chromosome %s Mb (resolution: %sKb)' % (schr, resolution / 1000))
            elif numOfrows <= rowcounter and randomBins:
                ax2.set_xlabel('Chromosome %s (Genomic Bins)' % (schr))

            if h_start > 0:
                for item in range(0, len(h_start)):
                    if dark:
                        ax2.axvspan(h_start[item], h_end[item], facecolor='#78D400', alpha=0.30, linestyle='dashed')
                    else:
        ...
        if anchor > 0:  # normalization can be useful
            ...
            if not tripleColumn:
                ...
                if dark:
                    ax3.plot(x_comps, fourClike, color='white')
                else:
                    ax3.plot(x_comps, fourClike, color='black')
            else:
                if dark:
                    ax3.plot(np.arange(start, end), fourClike, 'white')
                else:
                    ax3.plot(np.arange(start, end), fourClike, 'black')

            ax3.locator_params(axis='y', tight=False, nbins=3)
            ax3.set_xlim(int(start or 1) - 0.5, int(start or 1) + length - 0.5)
            if anchorMax > 0:
                ax3.set_ylim(0, anchorMax)
            else:
                ax3.set_ylim(0, max(fourClike) / 20.0)
            ...
            if h_start > 0:
                for item in range(0, len(h_start)):
                    if dark:
                        ax3.axvspan(h_start[item], h_end[item], facecolor='#78D400', alpha=0.30, linestyle='dashed')
                    else:
            ...
            if numOfrows <= rowcounter and not randomBins:
                ax3.set_xlabel('Chromosome %s Mb (resolution: %sKb)' % (schr, resolution / 1000))
            elif numOfrows <= rowcounter and randomBins:
                ax3.set_xlabel('Chromosome %s (Genomic Bins)' % (schr))
        ...
        if len(plotGenes) > 0:
            ...
            for item in genes.keys():
                ...
                if len(genes[item]) > 2:  # plot with introns
                    ...
                    ax3.plot([gstart, gend], [trackCount - gtrack + 0.125, trackCount - gtrack + 0.125], color=icolor,
                    ...
                    if plength <= 30:
                        ...
                        for exon in range(0, len(geend) - 1):
                            ...
                            if len(genes[item]) > 5:
                                grect = Rectangle((float(gestart[exon]) / resolution, trackCount - gtrack),
                                                  (float(geend[exon]) / resolution - float(gestart[exon]) / resolution),
                                                  0.25, color=genes[item][5])
                            else:
                                grect = Rectangle((float(gestart[exon]) / resolution, trackCount - gtrack),
                                                  (float(geend[exon]) / resolution - float(gestart[exon]) / resolution),
                                                  0.25, color=gcolor)
                            ...
                            if exon < len(geend) - 2:
                                if (float(gestart[exon + 1]) / resolution - float(geend[exon]) / resolution) > 0.5:
                                    if genes[item][2] == '-':
                                        ax3.plot(float(gestart[exon]) / resolution + (
                                                float(gestart[exon + 1]) / resolution - float(
                                            geend[exon]) / resolution) / 2 - 0.125, trackCount - gtrack + 0.125,
                                                 marker=arrow, color=icolor, markersize=1.25)
                                    else:
                                        ax3.plot(float(gestart[exon]) / resolution + (
                                                float(gestart[exon + 1]) / resolution - float(
                                            geend[exon]) / resolution) / 2 + 0.125, trackCount - gtrack + 0.125,
                        ...
                        if len(genes[item]) > 5:
                            rect = Rectangle((gstart, trackCount - gtrack), (gend - gstart), 0.25, color=genes[item][5])
                        else:
                            rect = Rectangle((gstart, trackCount - gtrack), (gend - gstart), 0.25, color=gcolor)
                        ax3.add_patch(rect)
                ...
                if plength <= 30 and geneLabels:  # also consider the gene density
                    ...
                    if gindex < len(nearest[gtrack]) - 1:
                        downgene = nearest[gtrack][gindex + 1]
                    else:
                        downgene = upgene

                    if plength <= 2 or plength < 1:
                        plength = 1
                    elif plength <= 4:
                        plength = 2
                    else:
                    ...
                    if len(genes[item]) > 6:
                        ax3.text(gstart, trackCount - gtrack + 0.5, genes[item][1], fontsize=genes[item][6],
                                 color=genes[item][5])
                    elif len(nearest[gtrack]) == 1:
                        ax3.text(gstart, trackCount - gtrack + 0.5, genes[item][1], fontsize=4.5 / plength)
                    elif float(gdist) / resolution >= 2 and len(genes[item][1]) <= 6:
                        ax3.text(gstart, trackCount - gtrack + 0.5, genes[item][1], fontsize=4.5 / plength)
                    elif float(gdist) / resolution >= 2 and len(genes[item][1]) > 6:
                        ax3.text(gstart, trackCount - gtrack + 0.5, genes[item][1], fontsize=3 / plength)
                    elif float(gdist) / resolution < 2 and float(gdist) / resolution > 1 and len(genes[item][1]) <= 6:
                        ax3.text(gstart, trackCount - gtrack + 0.5, genes[item][1], fontsize=2.5 / plength)
                    elif float(gdist) / resolution < 2 and float(gdist) / resolution > 1 and len(genes[item][1]) > 6:
                        ax3.text(gstart, trackCount - gtrack + 0.5, genes[item][1], fontsize=2 / plength)
                    elif float(gdist) / resolution <= 1 and float(gdist) / resolution >= 0.25:
                        ax3.text(gstart, trackCount - gtrack + 0.5, genes[item][1], fontsize=1.8)
                    else:
                        ax3.text(gstart, trackCount - gtrack + 0.5, genes[item][1], fontsize=1)
        ...
        if len(histograms) > 0:
            if not superImpose:  # improve this part
                for x in range(0, len(histograms[0].split(','))):
                    ax3 = plt.subplot2grid((numOfrows, 4 * len(files)), (rowcounter, exp * 4), rowspan=1, colspan=4,
                                           sharex=ax1)
                    ax3.get_yaxis().set_label_coords(-0.125, 0.5)
                    x_comps, x_comps2, y_comps, colors, texts = read_bedGraph(histograms[exp].split(',')[x], resolution,
                                                                              chromosome)

                    if dark:
                        ax3.plot(x_comps, y_comps, color='white')
                    else:
                        ax3.plot(x_comps, y_comps, color='black')

                    if exp == 0:
                        ystart, yend = where(start, end, x_comps)
                        ymin = min(y_comps[ystart:yend]) + min(y_comps[ystart:yend]) / 10 if min(
                            y_comps[ystart:yend]) < 0 else min(y_comps[ystart:yend]) - min(y_comps[ystart:yend]) / 10
                    ...
                    if len(fillHist) > 0 and int(fillHist[exp].split(',')[x]) == 1:
                        ...
                        if len(histColors) > 0:
                            if histColors[exp].split(',')[x] != '':
                                ax3.fill_between(x_comps, comps2, 0, color='#' + histColors[exp].split(',')[x],
                                                 interpolate=True)
                                if ymin < 0:
                                    with np.errstate(all='ignore'): ax3.fill_between(x_comps, comps2, 0, comps2 > 0,
                                                                                     color='#' +
                                                                                           histColors[exp].split(',')[
                                                                                               x], interpolate=True)
                                    with np.errstate(all='ignore'): ax3.fill_between(x_comps, comps2, 0,
                                                                                     where=comps2 < 0, color='black',
                                                                                     interpolate=True)
                            else:
                                ax3.fill_between(x_comps, comps2, 0, color='gray', interpolate=True)
                                if ymin < 0:
                                    with np.errstate(all='ignore'): ax3.fill_between(x_comps, comps2, 0, comps2 > 0,
                                                                                     color='gray', interpolate=True)
                                    with np.errstate(all='ignore'): ax3.fill_between(x_comps, comps2, 0,
                                                                                     where=comps2 < 0, color='black',
                                                                                     interpolate=True)
                        else:
                            ...
                            if ymin < 0:
                                with np.errstate(all='ignore'): ax3.fill_between(x_comps, comps2, 0, comps2 > 0,
                                                                                 color='gray', interpolate=True)
                                with np.errstate(all='ignore'): ax3.fill_between(x_comps, comps2, 0, where=comps2 < 0,
                                                                                 color='black', interpolate=True)
                    ...
                    if h_start > 0:
                        for item in range(0, len(h_start)):
                            if dark:
                                ax3.axvspan(h_start[item], h_end[item], facecolor='#78D400', alpha=0.30,
                                            linestyle='dashed')
                            else:
                                ax3.axvspan(h_start[item], h_end[item], facecolor='g', alpha=0.10, linestyle='dashed')
                ...
                if len(histColors) > 0:
                    if histColors[exp].split(',')[0] != '': ax3.plot(x_comps, y_comps,
                                                                     color='#' + histColors[exp].split(',')[0],
                                                                     alpha=0.5);firstcolor = '#' + \
                                                                                             histColors[exp].split(',')[
                                                                                                 0]
                elif dark:
                    ax3.plot(x_comps, y_comps, color='white', alpha=0.5)
                else:
                    ax3.plot(x_comps, y_comps, color='black', alpha=0.5)
                ...
                if len(fillHist) > 0 and int(fillHist[exp].split(',')[0]) == 1:
                    ...
                    if len(histColors) > 0:
                        if histColors[exp].split(',')[0] != '':
                            ax3.fill_between(x_comps, comps2, 0, color='#' + histColors[exp].split(',')[0],
                                             interpolate=True)
                            if ymin < 0:
                                with np.errstate(all='ignore'): ax3.fill_between(x_comps, comps2, 0, comps2 > 0,
                                                                                 color='#' + histColors[exp].split(',')[
                                                                                     0], interpolate=True)
                                with np.errstate(all='ignore'): ax3.fill_between(x_comps, comps2, 0, where=comps2 < 0,
                                                                                 color='black', interpolate=True)
                        else:
                ...
                if len(histColors) > 0:
                    if histColors[exp].split(',')[1] != '': ax3a.plot(x_comps, y_comps,
                                                                      color='#' + histColors[exp].split(',')[
                                                                          1]); subcolor = '#' + \
                                                                                          histColors[exp].split(',')[1]
                else:
                    ax3a.plot(x_comps, y_comps, color='red')
                ...
                if exp == 0:
                    ...
                    if len(histMax) == 0:
                        ...
                        ymaxlims.append(max(y_comps[ystart:yend]))
                        ax3a.text(int(start or 1) + length - length / 6,
                                  max(y_comps[ystart:yend]) - max(y_comps[ystart:yend]) / 10,
                                  histLabels[exp].split(',')[1], color=subcolor)
                    else:
                        ax3a.set_ylim(ymin, int(histMax[exp].split(',')[1]) + int(histMax[exp].split(',')[1]) / 10)
                        ax3a.text(int(start or 1) + length - length / 6,
                                  int(histMax[exp].split(',')[1]) - int(histMax[exp].split(',')[1]) / 10,
                                  histLabels[exp].split(',')[1], color=subcolor)
                ...
                if len(fillHist) > 0 and int(fillHist[exp].split(',')[1]) == 1:
                    ...
                    if len(histColors) > 0:
                        if histColors[exp].split(',')[1] != '':
                            ax3a.fill_between(x_comps, comps2, 0, color='#' + histColors[exp].split(',')[1],
                                              interpolate=True)
                            if ymin < 0:
                                with np.errstate(all='ignore'): ax3a.fill_between(x_comps, comps2, 0, comps2 > 0,
                                                                                  color='#' +
                                                                                        histColors[exp].split(',')[1],
                                                                                  interpolate=True)
                                with np.errstate(all='ignore'): ax3a.fill_between(x_comps, comps2, 0, where=comps2 < 0,
                                                                                  color='black', interpolate=True)
                        else:
                            ax3.fill_between(x_comps, comps2, 0, color='gray', interpolate=True)
                            if ymin < 0:
        ...
        if len(barPlots) > 0:
            for x in range(0, len(barPlots[0].split(','))):
                ...
                ax3.get_yaxis().set_label_coords(-0.125, 0.5)
                x_comps, x_comps2, y_comps, colors, texts = read_bedGraph(barPlots[exp].split(',')[x], resolution,
                                                                          chromosome)

                if len(barMax) == 0:
                    hMax = max(y_comps)
                else:
                    hMax = float(barMax[exp].split(',')[x])  # need to implement length check

                for item in range(0, len(x_comps)):
                    # if x_comps[item]>=start and x_comps[item]<=end:
                    if len(barMax) > 0 and y_comps[item] > float(barMax[exp].split(',')[x]): y_comps[item] = float(
                        barMax[exp].split(',')[x])
                    if len(barColors) == 0 and len(colors) == 0:
                        rect = Rectangle((x_comps[item], 0.0), (x_comps2[item] - x_comps[item]), y_comps[item],
                                         color='#0099FF', alpha=y_comps[item] / hMax)
                    elif len(colors) > 0:
                        rect = Rectangle((x_comps[item], 0.0), (x_comps2[item] - x_comps[item]), y_comps[item],
                                         color=colors[item])
                    elif len(barColors) > 0:
                        rect = Rectangle((x_comps[item], 0.0), (x_comps2[item] - x_comps[item]), y_comps[item],
                                         color='#' + barColors[exp].split(',')[x], alpha=y_comps[item] / hMax)
                    ax3.add_patch(rect)
                x_comps = [];
                x_comps2 = [];
                y_comps = [];
        ...
        if len(tilePlots) > 0:
            for x in range(0, len(tilePlots[0].split(','))):
                ...
                ax3.get_yaxis().set_label_coords(-0.125, 0.5)
                x_comps, x_comps2, y_comps, colors, texts = read_bedGraph(tilePlots[exp].split(',')[x], resolution,
                                                                          chromosome)
                for item in range(0, len(x_comps)):
                    if len(tileColors) == 0 and len(colors) == 0:
                        rect = Rectangle((x_comps[item], 0.35), (x_comps2[item] - x_comps[item]), 0.25, color='#0099FF')
                    elif len(colors) > 0:
                        rect = Rectangle((x_comps[item], 0.35), (x_comps2[item] - x_comps[item]), 0.25,
                                         color=colors[item])
                    elif len(tileColors) > 0:
                        rect = Rectangle((x_comps[item], 0.35), (x_comps2[item] - x_comps[item]), 0.25,
                                         color='#' + tileColors[exp].split(',')[x])
        ...
        if len(arcPlots) > 0:
            for x in range(0, len(arcPlots[0].split(','))):
                ...
                ax3.get_yaxis().set_label_coords(-0.125, 0.5)
                x_comps, x_comps2, y_comps, colors, texts = read_bedGraph(arcPlots[exp].split(',')[x], resolution,
                                                                          chromosome)
                ymax = 0
                ...
                for item in range(0, len(x_comps)):
                    ...
                    if dark and len(arcColors) == 0 and len(colors) == 0:
                        ax3.plot(pts[:, 0], pts[:, 1], c='#cecece')
                    elif len(arcColors) == 0 and len(colors) == 0:
                        ax3.plot(pts[:, 0], pts[:, 1], c='black')
                    elif len(colors) > 0:
                        ax3.fill_between(pts[:, 0], pts[:, 1], 0, color=colors[item], interpolate=True, alpha=0.35)
                    elif len(arcColors) > 0:
                        ax3.fill_between(pts[:, 0], pts[:, 1], 0, color='#' + arcColors[exp].split(',')[x],
        ...
        if len(epiLogos) > 0:
            if imputed:
                obs_colors = {'1': '#ff0000', '2': '#ff4500', '3': '#ff4500', '4': '#ff4500', '5': '#008000',
                              '6': '#008000', '7': '#008000', '8': '#009600', '9': '#c2e105', '10': '#c2e105',
                              '11': '#c2e105', '12': '#c2e105', '13': '#ffc34d', '14': '#ffc34d', '15': '#ffc34d',
                              '16': '#ffff00', '17': '#ffff00', '18': '#ffff00', '19': '#ffff66', '20': '#66cdaa',
                              '21': '#8a91d0', '22': '#e6b8b7', '23': '#7030a0', '24': '#808080', '25': '#ffffff'}
            else:
                obs_colors = {'1': '#ff0000', '2': '#ff4500', '3': '#32cd32', '4': '#008000', '5': '#006400',
                              '6': '#c2e105', '7': '#ffff00', '8': '#66cdaa', '9': '#8a91d0', '10': '#cd5c5c',
                              '11': '#e9967a', '12': '#bdb76b', '13': '#808080', '14': '#c0c0c0', '15': '#ffffff'}
        ...
        if plotInsulation:
            ...
            if h_start > 0:
                for item in range(0, len(h_start)):
                    if dark:
                        ax4.axvspan(h_start[item], h_end[item], facecolor='#78D400', alpha=0.30, linestyle='dashed')
                    else:
            ...
            rowcounter += 1
            if numOfrows <= rowcounter and not randomBins:
                ax4.set_xlabel('Chromosome %s Mb (resolution: %sKb)' % (schr, resolution / 1000))
            elif numOfrows <= rowcounter and randomBins:
                ax4.set_xlabel('Chromosome %s (Genomic Bins)' % (schr))
        ...
        if plotTadDomains:
            ...
            for item in range(0, len(tricks) - 1):
                ...
                if plotDomainsAsBars:
                    if not plotPublishedTadDomains:
                        p = Rectangle((tricks[item], 0.2), (tricks[item + 1] - tricks[item]), 0.25, color=tcolor,
                                      alpha=0.75)
                    else:
                        p = Rectangle((tricks[item], 0.1), (tricks[item + 1] - tricks[item]), 0.15, color=tcolor,
                                      alpha=0.75)
                else:
                    if not tripleColumn:
                        pts = np.array([[tricks[item], 0], [tricks[item + 1], 0],
                                        [floor((tricks[item] + tricks[item + 1]) / 2), 0.75]])
                        p = Polygon(pts, closed=True, color=tcolor,
                                    alpha=max(nums[tricks[item]:tricks[item + 1]]) / max(nums))
                    else:
                        pts = np.array([[tricks[item], 0], [tricks[item + 1], 0],
                                        [floor((tricks[item] + tricks[item + 1]) / 2), 0.75]])
                        p = Polygon(pts, closed=True, color=tcolor)

                if not tripleColumn and sum(nums[slice(tricks[item], tricks[item + 1])]) > np.percentile(np.array(nums),
                                                                                                         75):
                    ax5.add_patch(p)
                elif tripleColumn and sum(nums[slice(tricks[item] - start, tricks[item + 1] - start)]) > np.percentile(
                        np.array(nums), 75):
                    ax5.add_patch(p)
            ...
            if numOfrows <= rowcounter and not plotCustomDomains and not randomBins:
                ax5.set_xlabel('Chromosome %s Mb (resolution: %sKb)' % (schr, resolution / 1000))
            elif numOfrows <= rowcounter and not plotCustomDomains and randomBins:
                ax5.set_xlabel('Chromosome %s (Genomic Bins)' % (schr))
            ax5.set_ylim(0, 0.75)
            if spine > 0:
        ...
        if plotCustomDomains and not plotTadDomains:
            ...
            for item in range(0, len(x_comps)):
                ...
                if len(colors) > 0:
                    tcolor = colors[item]
                elif len(domColors) > 0:
                    tcolor = '#' + domColors[exp]

                if plotDomainsAsBars:
                    if not plotPublishedTadDomains:
                        p = Rectangle((x_comps[item], 0.2), (x_comps2[item] - x_comps[item]), 0.25, color=tcolor,
                                      alpha=0.75)
                    else:
                        p = Rectangle((x_comps[item], 0.1), (x_comps2[item] - x_comps[item]), 0.15, color=tcolor,
                                      alpha=0.75)
                else:
                    pts = np.array(
                        [[x_comps[item], 0], [x_comps2[item], 0], [floor((x_comps[item] + x_comps2[item]) / 2), 0.75]])
    ...
    if compare and not pair:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            matrix = matrix1 / (matrix2 * 1.0)
        # matrix[np.logical_and(matrix>=0.5, matrix<=1)]=1
        ax1 = plt.subplot2grid((numOfrows, 4 * len(files)), (0, (exp + 1) * 4), rowspan=4, colspan=4)
        with np.errstate(divide='ignore'):
            img = ax1.imshow(log2(matrix), cmap=plt.get_cmap("bwr"), origin=orientation, interpolation="nearest",
                             extent=(int(start or 1) - 0.5, \
                                     int(start or 1) + length - 0.5, int(start or 1) - 0.5,
                                     int(start or 1) + length - 0.5), aspect='auto')
        ...
        divider = make_axes_locatable(ax1)
        if len(compareEx) > 0:
            clow = int(compareEx.split(',')[0]) * -1;
            chigh = int(compareEx.split(',')[1]);
            img.set_clim([clow, chigh])
        else:
            img.set_clim([-4, 4])
        ...
        if plotTriangular:
            ...
            ax2 = plt.subplot2grid((numOfrows, 4 * len(files)), (crowcounter, (exp + 1) * 4), rowspan=2, colspan=4,
                                   sharex=ax1)
            dst = ndimage.rotate(matrix, 45, order=0, reshape=True, prefilter=False, cval=0)
            matrix = [];
            if not triangularHeight:
            ...
            ax2.set(adjustable='box-forced')
            with np.errstate(divide='ignore'):
                img = ax2.imshow(log2(dst), cmap=plt.get_cmap("bwr"), origin=orientation, interpolation="nearest",
                                 extent=(int(start or 1) - 0.5, \
                                         int(start or 1) + length - 0.5, int(start or 1) - 0.5,
                                         int(start or 1) + length - 0.5), aspect='auto')
            dst = [];
            ...
            if plotTadDomains and plotDomainTicks:
                ...
                ax2.xaxis.grid(True, which='minor', linewidth=2)
            crowcounter += 2
            if numOfrows <= crowcounter and not randomBins:
                ax2.set_xlabel('Chromosome %s Mb (resolution: %sKb)' % (schr, resolution / 1000))
            elif numOfrows <= crowcounter and randomBins:
                ax2.set_xlabel('Chromosome %s (Genomic Bins)' % (schr))
        ...
        if len(plotGenes) > 0:
            ...
            ax3 = plt.subplot2grid((numOfrows, 4 * len(files)), (crowcounter, (exp + 1) * 4), rowspan=2, colspan=4,
                                   sharex=ax1)
            if exp == 0: ax3.set_ylabel('Genes')
            ax3.get_yaxis().set_label_coords(-0.125, 0.5)
            genes, trackCount, nearest = read_genes(plotGenes[0], resolution, chromosome, start, end)
        ...
        for peer1 in range(0, len(files)):
            ...
            for peer2 in range(0, len(files)):
                ...
                if peer1 != peer2:
                    ...
                    pax1 = plt.subplot2grid((numOfrows, 4 * len(files)), (prowcounter, peer1 * 4), rowspan=4, colspan=4,
                                            sharex=ax1)
                    with np.errstate(divide='ignore'):
                        img = pax1.imshow(log2(matrix), cmap=plt.get_cmap("bwr"), origin=orientation,
                                          interpolation="nearest", extent=(int(start or 1) - 0.5, \
                                                                           int(start or 1) + length - 0.5,
                                                                           int(start or 1) - 0.5,
                                                                           int(start or 1) + length - 0.5),
                    ...
                    if h_start > 0:
                        for item in range(0, len(h_start)):
                            if dark:
                                pax1.axvspan(h_start[item], h_end[item], facecolor='#78D400', alpha=0.30,
                                             linestyle='dashed')
                            else:
                    ...
                    cax = divider.append_axes("bottom", size="2.5%", pad=0.1)
                    cbar = plt.colorbar(img, cax=cax, ticks=MultipleLocator(2.0), format="%.1f",
                                        orientation='horizontal', extendfrac='auto', spacing='uniform')
    ...
    if len(oExtension) > 0 and oExtension in plt.gcf().canvas.get_supported_filetypes().keys():
        extension = '.' + oExtension
    elif 'JPEG' in plt.gcf().canvas.get_supported_filetypes_grouped().keys() or 'Joint Photographic Experts Group' in plt.gcf().canvas.get_supported_filetypes_grouped().keys():
        extension = '.jpeg'
    else:
        extension = '.png'
    ...
    if wholeGenome:
        ...
        if highResolution and dPixels == 200:
            plt.savefig(
                output + '-' + chromosome + '.' + 'ofBins(' + str(start) + '-' + str(end) + ').RandomBins' + extension,
                dpi=200)
        elif dPixels != 200:
            plt.savefig(
                output + '-' + chromosome + '.' + 'ofBins(' + str(start) + '-' + str(end) + ').RandomBins' + extension,
        ...
        if highResolution and dPixels == 200:
            plt.savefig(output + '-' + chromosome + '.' + 'ofBins(' + str(start) + '-' + str(end) + ').' + str(
                resolution / 1000) + 'K' + extension, dpi=200)
        elif dPixels != 200:
            plt.savefig(output + '-' + chromosome + '.' + 'ofBins(' + str(start) + '-' + str(end) + ').' + str(
                resolution / 1000) + 'K' + extension, dpi=dPixels)
        else:
            plt.savefig(output + '-' + chromosome + '.' + 'ofBins(' + str(start) + '-' + str(end) + ').' + str(
                resolution / 1000) + 'K' + extension)


if __name__ == '__main__':

    parser = argparse.ArgumentParser(usage='HiCPlotter.py -f file1 file2 ... -n name1 name2 ... -chr chr12 -o hES',
    ...
    group1.add_argument('-hmc', '--heatmapColor', type=int, default=3, metavar='',
                        help="Colors for heatmap: Greys(0), Reds(1), YellowToBlue(2), YellowToRed(3-default), Hot(4), BlueToRed(5)")
    ...
    group1.add_argument('-hR', '--highResolution', type=int, default=True, metavar='',
                        help="default: 1 - disable with 0")
    group1.add_argument('-pi', '--plotInsulation', type=int, default=False, metavar='',
                        help="default: 0 - enable with 1")
    group1.add_argument('-dc', '--domColors', nargs='+', metavar='', default=[])
    group1.add_argument('-ptr', '--plotTriangular', type=int, default=False, metavar='',
                        help="default: 0 - enable with 1")
    group1.add_argument('-trh', '--triangularHeight', type=int, default=False, metavar='',
                        help="default: (end-start)/5")
    group1.add_argument('-ptd', '--plotTadDomains', type=int, default=False, metavar='',
                        help="default: 0 - enable with 1")
    group1.add_argument('-pdt', '--plotDomainTicks', type=int, default=True, metavar='',
                        help="default: 1 - enable with 0")
    group1.add_argument('-pcd', '--plotCustomDomains', type=int, default=False, metavar='',
                        help="default: 0 - enable with 1")
    group1.add_argument('-pdb', '--plotDomainsAsBars', type=int, default=False, metavar='',
                        help="default: 0 - enable with 1")
    group1.add_argument('-pcdf', '--customDomainsFile', nargs='+', metavar='', default=[])
    group1.add_argument('-pptd', '--plotPublishedTadDomains', type=int, default=False, metavar='',
                        help="default: 0 - enable with 1")
    group1.add_argument('-ptdo', '--publishedTadDomainOrganism', type=int, default=True, metavar='',
                        help="human(default): 1 - mouse: 0")
    ...
    if args['verbose']:
        logging.basicConfig(filename=args['output'] + '.log', level=logging.DEBUG, format='%(asctime)s %(message)s',
