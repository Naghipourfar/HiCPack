...
for l in open(sys.argv[1]).read().split('\n'):
    ...
    if len(l) > 2:
        ...
        if l[0] == 'M' and l[1] == '=':
            M = int(l[2])
        if l[0] == 'gamma' and l[1] == '=':
            gamma = int(l[2])
        if l[0] == 'output_directory' and l[1] == '=':
            output_directory = l[2]
        if l[0] == 'contact_map_path' and l[1] == '=':
            paths = [x for x in l[2].split(',')]
        if l[0] == 'contact_map_name' and l[1] == '=':
            chrs = [x for x in l[2].split(',')]
        if l[0] == 'N' and l[1] == '=':
            N = [int(x) for x in l[2].split(',')]

if S is None:
    print('Haven\'t specified parameter: "S"')
if p is None:
    print('Haven\'t specified parameter: "p"')
if q is None:
    print('Haven\'t specified parameter: "q"')
if M is None:
    print('Haven\'t specified parameter: "M"')
if gamma is None:
    print('Haven\'t specified parameter: "gamma"')
if output_directory is None:
    print('Haven\'t specified parameter: "output_directory"')

if len(chrs) != len(paths) or len(N) != len(paths):
    print('Number of contact map paths, contact map names, and N values is inconsistent')
if len(paths) == 0:
    print('No contact map paths specified')
                                ...
                                if olddelta > gmat[i + k - l, i + k]:
                                    ...
                                    for z in range(l - 1):
                                        for w in range(z + 1, l):
                                            oldscore += ((olddelta * float(w - z) + oldbeta) * backgrnd[w - z] - mat[
                                                i + k - l + z, i + k - l + w]) ** 2

                                    options[l, tt] = local_score[k - l, t - tt - 1] + score[i + k - l, i + k, tt] - (
                                            oldscore - bakmat[i + k - l, i + k])
    ...
    for i in range(mat.shape[0]):
        for j in range(mat.shape[1]):
            if bi[i] < 0 or bi[j] < 0:
                ...
                smat[i, j] = smat[i, j] - gamma * (bi[i] + bi[j])
