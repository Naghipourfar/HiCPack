...
if __name__ == "__main__":
    ...
    parser.add_argument("-b", "--bins",
                        help='BED file with bins coordinates. If provided the chromosome lengths are used to define the output matrix size')
    parser.add_argument("-d", "--di",
                        help="If specified the output matrix is formatted for Dixon et al. TADs calling. In this case --bins is required",
                        action='store_true')
    parser.add_argument("-c", "--perchr",
                        help="If specified intrachromosomal maps are written per chromosome as individual dense matrices. In this case, --bins must also be specified",
                        action='store_true')
    ...
    ## Per chromosome matrices
        ...
        for i in range(1, len(lc)):
            ...
            counts_perchr = sparse.coo_matrix(
                (counts.data[idxintra], (counts.row[idxintra] - lc[i - 1], counts.col[idxintra] - lc[i - 1])),
